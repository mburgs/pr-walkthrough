...
def format_hunk_for_plan(hunk: Hunk) -> str:
    """Render one Hunk as a compact block suitable for the planning prompt."""
    return (
        f"File: {hunk.file}\n"
...
def build_plan_tour_user_message(pr: PRMetadata, diff: list[Hunk]) -> str:
    ...
    Intent: Give Claude a complete picture of the PR — its metadata, the
    author's description, and every hunk — so it can choose an ordering
    that reveals the change architecturally rather than alphabetically.
    The session_id is generated by the caller; we request the planner to
    embed it verbatim.
    """
    diff_text = "\n---\n".join(format_hunk_for_plan(h) for h in diff)
...
TASK
----
Return a TourPlan: an ordered list of TourChunks that walks the reviewer \
through this change in the order that makes the most architectural sense. \
Lead with the core mechanism that everything else depends on. Group related \
hunks into the same chunk when they form a single logical unit. Each chunk \
needs:

- chunk_id: "c1", "c2", … (sequential, no gaps)
- files: list of files touched
- hunks: the exact Hunk objects from the diff that belong to this chunk \
  (copy them verbatim — do not alter file, old_range, new_range, header, or body)
- summary: one tight sentence shown in the chunk list UI
- rationale_for_position: one sentence explaining why this chunk appears \
  at this position in the tour
- est_concern_level: "low" | "medium" | "high"

The session_id field must be set to the literal string "PENDING" — \
the orchestrator will replace it. The pr field must echo the PR metadata \
provided above verbatim (all fields).
"""
