...
async def test_over_referenced_symbol_suppresses_callsites(tmp_path: Path) -> None:
    """A symbol used in tens of places (imported enum / shared base
    class) shouldn't fill the related-code panel with its callsites.
    The retriever keeps its definition but drops its references when
    the LSP returns more than OVER_REFERENCED_THRESHOLD sites."""
    from pr_walkthrough.context.lsp.retriever import OVER_REFERENCED_THRESHOLD

    src = tmp_path / "src.py"
    src.write_text("Transparency.OPAQUE\n")
    def_path = tmp_path / "core" / "models.py"
    def_path.parent.mkdir()
    def_path.write_text("class Transparency: ...\n")
    # 25 fake reference sites — over the threshold
    ref_paths = []
    for i in range(OVER_REFERENCED_THRESHOLD + 5):
        p = tmp_path / f"other_{i}.py"
        p.write_text("Transparency.OPAQUE\n")
        ref_paths.append(p)

    lookups = {
        "Transparency": {
            "definition": [{"uri": def_path.as_uri(), "range": {"start": {"line": 0}}}],
            "references": [
                {"uri": p.as_uri(), "range": {"start": {"line": 0}}} for p in ref_paths
            ],
        },
    }
    retriever = LSPContextRetriever(_FakePool(lookups))
    ...
    kinds = [h.relationship for h in hits]
    assert "definition" in kinds, "definition should still surface"
    assert "callsite" not in kinds, (
        f"over-referenced symbol's callsites should be dropped; got {kinds}"
    )


async def test_per_seed_reference_cap(tmp_path: Path) -> None:
    """Even a normally-referenced symbol shouldn't take more than
    REFS_PER_SEED spots — otherwise the first seed can swallow the
    whole quota before other identifiers get a turn."""
    from pr_walkthrough.context.lsp.retriever import REFS_PER_SEED

    src = tmp_path / "src.py"
    src.write_text("widget()\n")
    # 8 callsites — under the over-referenced threshold, but more
    # than the per-seed cap
    ref_paths = []
    for i in range(8):
    ...
    anchor = CodeAnchor(file="src.py", line_range=(1, 1))
    hits = await retriever.related(anchor, tmp_path)
    callsites = [h for h in hits if h.relationship == "callsite"]
    assert len(callsites) == REFS_PER_SEED
